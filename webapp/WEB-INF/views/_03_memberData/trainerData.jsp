        ...
        <div class="content">
            <div class="title"><h3>帳號設定</h3></div>
                ...
                <div class="setting_area">
                        <form:form action="/TrainMe/updateTrData/${trainerBean.id}" method="post" modelAttribute="trainerBean" enctype="multipart/form-data">
                            <div class="setting_box">
                                <label>姓名</label><p>${trainerBean.name}</p>
                            </div>
                            <div class="setting_box">
                                <label>Email</label><p>${trainerBean.email}</p>
                            </div>
                            <div class="setting_box">
                                <label>密碼</label><a href="<c:url value='/_03MemberData/passwordUpdate.jsp' />">修改密碼</a>
                            </div>
                            <div class="setting_box">
                                <label>生日</label><p>${trainerBean.birthday}</p>
                            </div>
                            <div class="setting_box">
                                <label>健身房</label><p>${gymName}</p>
                            </div>
                            <div class="setting_box">
                                <label>手機</label>
                                <input type="text" required name="phone" value="${trainerBean.phone}">
                            </div>
                            <div class="setting_box">
                                <label>顯示暱稱</label>
                                <input type="text" required name="nickname" value="${trainerBean.nickname}">
                            </div>
                            ...
                            <div class="setting_box address">
                               ...
                               <form:select path='city.id'>
                                ...
                                <input type="text" name="address" placeholder="地址" value="${trainerBean.address}">
                            </div>
                             <div class="setting_box">
                                <label>銀行帳戶</label>
                                <input type="text" required name="bank_account" value="${trainerBean.bank_account}">
                            </div>
                            ...
                            <div class="setting_box upload_file">
                                ...
                                <div class="upload_file_area">
                                    <div id="imgbox" class="ufl_box upload_imgbox">
									    ...
									    <c:otherwise>
									       <img id="photo" name="photo" src="/upload/${trainerBean.profile_image}">
									    </c:otherwise>
									</c:choose>
...
<!--                                                 <input type="file" accept="image/*" id="theFile" name="photo" /> accept : 限制上傳檔案類型 	 -->
                                               <form:input path="trainerImage" type='file' class="theFile"/>
	   	  	 								   <form:errors path="trainerImage"  cssClass="error" />
                                                上傳檔案
                                            </label>
                                        </button>
                                    </div>
...
//       var gyms = res.data;
//         console.log(gyms.id);
//         console.log(${LoginOK.gym});
//         console.log(gyms.id == ${LoginOK.gym});
//         for(gym of gyms){
// 	        if(gym.id == ${LoginOK.gym}){
// 	        	console.log('ok');
